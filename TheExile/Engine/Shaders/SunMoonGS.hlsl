...
void main(point TParticleToGS input[1], inout TriangleStream< GSOutput > OutputStream)
{
	...
	float3 ToObject = input[0].m_fCoordinate.xyz - cameraPos;
	ToObject = normalize(ToObject);
	float3 upVector = float3(fViewMatrix[0][1], fViewMatrix[1][1], fViewMatrix[2][1]);
		float3 rightVector = cross(ToObject, upVector);
		rightVector.y = 0.0f;
