...
inline void CreateProjectionMatrix(float fNearPlane, float fFarPlane, float fFOV, float fRatio, XMFLOAT4X4& fProjectionMatrix)
{
	float yScale = 1 / (tan(RADIANS(fFOV) / 2));
...
inline void CreateOrthoProjectionMatrix(float fNearPlane, float fFarPlane, XMFLOAT4X4& fProjectionMatrix)
{
	XMStoreFloat4x4(&fProjectionMatrix, XMMatrixOrthographicLH((float)WINDOW_WIDTH, (float)WINDOW_HEIGHT, fNearPlane, fFarPlane));
}